...
import (
	...
	. "github.com/s00500/env_logger/internal/testutils"
...
func TestInfoWithLines(t *testing.T) {
	LogAndAssertJSON(t, func(log logrus.FieldLogger) {
		...
		assert.Equal(t, "test", fields["msg"])
		assert.Equal(t, "info", fields["level"])
		assert.Equal(t, "/main_test.go", fields["file"])
	})
}
...
/*
// FIXME: we should support that I guess ? not sure yet...
...
func TestReportCallerWhenConfigured(t *testing.T) {
	...
	LogAndAssertText(t, func(log logrus.FieldLogger) {
		log.ReportCaller = true
...
func TestTimeOverrideMultipleLogs(t *testing.T) {
	var buffer bytes.Buffer
	var firstFields, secondfields logrus.Fields
